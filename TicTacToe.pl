% TICTACTOE Game
%
%Creating "win" where there are the ways to win the game
win(Brd, Plyr) :- rwin(Brd, Plyr);
                  cwin(Brd, Plyr);
                  dwin(Brd, Plyr).
%Here we make the ways to win, vertical, horizontal, diagonal
...
%Creating the board and his spaces
...
%Here we make the initializer
go :- how_to_play, strt([a,a,a,a,a,a,a,a,a]).
%First showing
how_to_play :-
  write('You are x player, enter a number.'),
  nl,
  disp([1,2,3,4,5,6,7,8,9]).

%Declaring the winner and how will show when someone wins
